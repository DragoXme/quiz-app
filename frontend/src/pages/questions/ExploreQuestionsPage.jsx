...
const ExploreQuestionsPage = () => {
    ...
    useEffect(() => { fetchTags(); }, []);
    useEffect(() => { fetchQuestions(); }, [currentPage, selectedTagIds, sortBy, sortOrder, filterTypes]);
    ...
    const fetchQuestions = async () => {
        ...
        try {
            ...
            if (filterTypes.length > 0) params.append('filterTypes', JSON.stringify(filterTypes));
            ...
            <div style={{ maxWidth: '860px', margin: '0 auto', padding: isMobile ? '16px' : '32px 24px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', gap: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div>
                            <h1 style={{ fontSize: isMobile ? '18px' : '24px', fontWeight: '800', color: 'var(--text-primary)' }}>Explore Questions</h1>
                            <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px' }}>{totalCount} question{totalCount !== 1 ? 's' : ''} found</p>
                        </div>
                    </div>
                    ...
                    }}>🔧 {isMobile ? 'Filter' : 'Filters & Sort'}</button>
                </div>
                    ...
                    <div style={glassCard}>
                        ...
                        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                            ...
                            <div style={{ flex: 1, minWidth: '120px' }}>
                                ...
                                <CustomSelect value={sortOrder} onChange={v => { setSortOrder(v); setCurrentPage(1); }} options={sortOrderOptions} />
                            </div>
                        </div>
                    </div>
                )}
                ...
                {/* Active Filters */}
                {(selectedTagIds.length > 0 || sortBy || filterTypes.length > 0) && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '14px', alignItems: 'center' }}>
                        ...
                        {filterTypes.includes('struggling') && <span style={{ padding: '2px 8px', backgroundColor: 'var(--error-light)', color: 'var(--error)', borderRadius: '20px', fontSize: '11px', fontWeight: '600' }}>🔴 Struggling</span>}
                        {filterTypes.includes('unattempted') && <span style={{ padding: '2px 8px', backgroundColor: 'var(--warning-light)', color: 'var(--warning)', borderRadius: '20px', fontSize: '11px', fontWeight: '600' }}>⏭️ Unattempted</span>}
                ...
                {loading ? (
                    ...
                    <div style={{ ...glassCard, textAlign: 'center', padding: '48px' }}>
                        ...
                        <button onClick={() => navigate('/questions/create')} style={{ marginTop: '16px', padding: '10px 24px', background: 'var(--gradient-accent)', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>
                    ...
                    <>
                        {questions.map((q, idx) => (
                            <div key={q.id} onClick={() => navigate(`/questions/${q.id}`)} style={{
                                ...
                                padding: isMobile ? '14px' : '18px', marginBottom: '10px',
                            ...
                            >
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                                    {q.tags?.filter(t => t.name !== 'starred' && t.name !== 'mcq single correct' && t.name !== 'mcq multiple correct' && t.name !== 'fill in the blank').map(tag => (
                                        <span key={tag.id} style={{ padding: '2px 6px', borderRadius: '10px', backgroundColor: 'var(--tag-bg)', color: 'var(--tag-text)', fontSize: '10px', fontWeight: '500' }}>{tag.name}</span>
                                    ))}
                                </div>
                            </div>
                        ))}
